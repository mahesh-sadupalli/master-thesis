...
metrics = train_model(
    ...
    device=device,
    num_epochs=EPOCHS,
    model_name='large_model_offline',
    output_dir=OUTPUT_DIR
)

export_metrics_csv(metrics, f"{OUTPUT_DIR}/large_model_metrics.csv")
...
plt.suptitle('Large Model (128-128-64) - Offline Training',
             fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/large_model_training_progress.png", dpi=300, bbox_inches='tight')
print(f"Training plot saved: {OUTPUT_DIR}/large_model_training_progress.png")
plt.close()

print("Large model offline training completed")
