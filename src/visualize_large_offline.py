"""
Large Model Offline Visualization

Generates spatio-temporal visualizations and error analysis for trained large model
with normalized colorbars in [0, 1] range.
"""
...
import json
import os
from unified_training_utils import SpatioTemporalDataset, LargeCompressor, compute_psnr_ssim

DATA_FILE = "../data/ML_test_loader_original_data.csv"
MODEL_DIR = "../results/large_model_offline"
OUTPUT_DIR = "../results/large_model_offline"
TIMESTEP = 0.0396

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Large model offline visualization")
print(f"Device: {device}")

dataset = SpatioTemporalDataset(DATA_FILE)

model = LargeCompressor().to(device)
model.load_state_dict(torch.load(f"{MODEL_DIR}/large_model_offline.pth", map_location=device))
model.eval()
...
plt.suptitle(f'Large Model (128-128-64) - PSNR: {psnr:.2f} dB - Timestep: {TIMESTEP}',
...
evaluation_results = {
    'model_file': 'large_model_offline.pth',
...
print("Large model visualization completed")
