"""
Medium Model Online Visualization

Generates spatio-temporal visualizations and error analysis for trained medium model
from online streaming training approach.
"""
...
import json
import os
from unified_training_utils import SpatioTemporalDataset, MediumCompressor, compute_psnr_ssim

DATA_FILE = "../data/ML_test_loader_original_data.csv"
MODEL_DIR = "../results/medium_model_online"
...
print("Medium model online visualization")
print(f"Device: {device}")

dataset = SpatioTemporalDataset(DATA_FILE)

model = MediumCompressor().to(device)
model.load_state_dict(torch.load(f"{MODEL_DIR}/medium_model_online_final.pth", map_location=device))
model.eval()
...
plt.suptitle(f'Medium Model (96-96-48) Online - PSNR: {psnr:.2f} dB - Timestep: {TIMESTEP}',
...
evaluation_results = {
    'model_file': 'medium_model_online_final.pth',
...
print("Medium model online visualization completed")
