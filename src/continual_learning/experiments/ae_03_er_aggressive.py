"""
Online Linear Autoencoder — Aggressive Experience Replay

Uses maximum buffer (100K) and full replay weight (1.0) for strongest
forgetting mitigation during online AE training.
"""
...
from continual_learning.experiments.ae_config import (
    ...
    AE_ONLINE_MODEL_CONFIGS, AE_OFFLINE_REFERENCE, ER_AGGRESSIVE_DEFAULTS
)


def run(model_name="base"):
    """Run Aggressive ER experiment for a single model size."""
    device = get_device()
    ...
    output_dir = os.path.join(RESULTS_BASE, "er_aggressive_{}".format(model_name))
    ...
    df.to_csv(os.path.join(output_dir, "{}_er_aggressive_metrics.csv".format(model_name)),
              index=False)
...
def run_all():
    """Run Aggressive ER across all model sizes."""
    results = []
    for model_name in AE_ONLINE_MODEL_CONFIGS:
        result = run(model_name)
        results.append(result)

    print("\n[Summary] Online AE — ER Aggressive (buffer={}, weight={})".format(
        ER_AGGRESSIVE_DEFAULTS['buffer_size'], ER_AGGRESSIVE_DEFAULTS['replay_weight']))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Online AE — Aggressive Experience Replay")
