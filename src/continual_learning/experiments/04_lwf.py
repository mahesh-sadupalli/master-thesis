...
Uses the model's predictions on new data before training as soft targets,
adding a distillation loss to maintain prior knowledge (Li and Hoiem, 2018).
"""
...
from continual_learning.cl_strategies import LwFStrategy
...
from continual_learning.experiments.config import (
    ...
    LWF_DEFAULTS, OFFLINE_REFERENCE
)
...
def run(model_name="base"):
    """Run Learning without Forgetting experiment for a single model size."""
    device = get_device()
    ...
    strategy = LwFStrategy(**LWF_DEFAULTS)
    output_dir = os.path.join(RESULTS_BASE, f"{model_name}_lwf")

    # Train
    metrics = train_online_cl(
        model=model, dataset=dataset, device=device,
        epochs_per_window=EPOCHS_PER_WINDOW,
        model_name=f"{model_name}_lwf",
    ...
    df.to_csv(os.path.join(output_dir, f"{model_name}_lwf_metrics.csv"), index=False)

    # Full dataset evaluation
    full_eval = evaluate_full_dataset(
        model, dataset, device, model_name=f"{model_name}_lwf"
    )

    # Save evaluation
    eval_data = {
        "model": f"{model_name}_lwf",
...
def run_all():
    """Run Learning without Forgetting experiment across all model sizes."""
    results = []
    for model_name in MODELS:
        result = run(model_name)
        results.append(result)

    print(f"\n[Summary] Learning without Forgetting (distill_weight={LWF_DEFAULTS['distill_weight']})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Learning without Forgetting (LwF)"
    )
