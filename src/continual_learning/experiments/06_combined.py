"""
Multi-strategy Hybrid: ER + LwF + EWC.

Combines replay buffer (data-level), knowledge distillation (output-level),
and weight regularization (parameter-level) for comprehensive forgetting
mitigation.
"""
...
from continual_learning.cl_strategies import CombinedStrategy
...
from continual_learning.experiments.config import (
    ...
    COMBINED_DEFAULTS, OFFLINE_REFERENCE
)
...
def run(model_name="base"):
    """Run Combined ER+LwF+EWC experiment for a single model size."""
    device = get_device()
    ...
    strategy = CombinedStrategy(**COMBINED_DEFAULTS)
    output_dir = os.path.join(RESULTS_BASE, f"{model_name}_combined")

    # Train
    metrics = train_online_cl(
        model=model, dataset=dataset, device=device,
        epochs_per_window=EPOCHS_PER_WINDOW,
        model_name=f"{model_name}_combined",
    ...
    df.to_csv(os.path.join(output_dir, f"{model_name}_combined_metrics.csv"), index=False)

    # Full dataset evaluation
    full_eval = evaluate_full_dataset(
        model, dataset, device, model_name=f"{model_name}_combined"
    )
...
def run_all():
    """Run Combined ER+LwF+EWC experiment across all model sizes."""
    results = []
    for model_name in MODELS:
        result = run(model_name)
        results.append(result)

    print(f"\n[Summary] Combined ER+LwF+EWC (buffer={COMBINED_DEFAULTS['buffer_size']}, "
          f"replay={COMBINED_DEFAULTS['replay_weight']}, "
          f"distill={COMBINED_DEFAULTS['distill_weight']}, "
          f"ewc_lambda={COMBINED_DEFAULTS['ewc_lambda']})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Combined ER+LwF+EWC multi-strategy hybrid"
    )
