...
def run(model_name="base"):
    """Run naive sequential baseline experiment for a single model size."""
    device = get_device()
    ...
    output_dir = os.path.join(RESULTS_BASE, f"{model_name}_naive")
    ...
    df.to_csv(os.path.join(output_dir, f"{model_name}_naive_metrics.csv"), index=False)

    # Full dataset evaluation
    full_eval = evaluate_full_dataset(
        model, dataset, device, model_name=f"{model_name}_naive"
    )

    # Save evaluation
    eval_data = {
        "model": f"{model_name}_naive",
...
def run_all():
    """Run naive sequential baseline experiment across all model sizes."""
    results = []
    for model_name in MODELS:
        result = run(model_name)
        results.append(result)

    print(f"\n[Summary] Naive Sequential Baseline")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Naive sequential baseline (no forgetting mitigation)"
    )
