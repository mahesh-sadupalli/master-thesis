"""
Hybrid: Dark Experience Replay++ (DER++).

Combines experience replay with stored model predictions (logits), applying
both task loss and distillation loss on replayed samples (Buzzega et al., 2020).
"""
...
from continual_learning.cl_strategies import DERppStrategy
...
from continual_learning.experiments.config import (
    ...
    DERPP_DEFAULTS, OFFLINE_REFERENCE
)
...
def run(model_name="base"):
    """Run Dark Experience Replay++ experiment for a single model size."""
    device = get_device()
    ...
    strategy = DERppStrategy(**DERPP_DEFAULTS)
    output_dir = os.path.join(RESULTS_BASE, f"{model_name}_der_pp")

    # Train
    metrics = train_online_cl(
        model=model, dataset=dataset, device=device,
        epochs_per_window=EPOCHS_PER_WINDOW,
        model_name=f"{model_name}_der_pp",
    ...
    df.to_csv(os.path.join(output_dir, f"{model_name}_der_pp_metrics.csv"), index=False)

    # Full dataset evaluation
    full_eval = evaluate_full_dataset(
        model, dataset, device, model_name=f"{model_name}_der_pp"
    )

    # Save evaluation
    eval_data = {
        "model": f"{model_name}_der_pp",
...
def run_all():
    """Run Dark Experience Replay++ experiment across all model sizes."""
    results = []
    for model_name in MODELS:
        result = run(model_name)
        results.append(result)

    print(f"\n[Summary] Dark Experience Replay++ (buffer={DERPP_DEFAULTS['buffer_size']}, "
          f"replay={DERPP_DEFAULTS['replay_weight']}, distill={DERPP_DEFAULTS['distill_weight']})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Dark Experience Replay++ (DER++)"
    )
