"""
Online Linear Autoencoder — Naive Baseline (No Replay)

Sequential fine-tuning across temporal windows with no forgetting mitigation.
Establishes the baseline for catastrophic forgetting in online AE training.
"""
...
from continual_learning.cl_strategies import NaiveStrategy
...
from continual_learning.experiments.ae_config import (
    ...
    AE_ONLINE_MODEL_CONFIGS, AE_OFFLINE_REFERENCE
)


def run(model_name="base"):
    """Run Naive baseline for a single model size."""
    device = get_device()
    ...
    df.to_csv(os.path.join(output_dir, "{}_naive_metrics.csv".format(model_name)),
              index=False)

    # Full dataset evaluation
    full_eval = evaluate_full_dataset_ae(
        model, dataset, device, model_name="{}_naive".format(model_name)
    )

    # Save evaluation
    eval_data = {
        "model": "{}_naive".format(model_name),
...
def run_all():
    """Run Naive baseline across all model sizes."""
    results = []
    for model_name in AE_ONLINE_MODEL_CONFIGS:
        result = run(model_name)
        results.append(result)

    print("\n[Summary] Online AE — Naive Baseline")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Online AE — Naive Baseline")
