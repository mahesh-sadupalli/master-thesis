"""
Medium Model Offline Visualization
...
MODEL_DIR = "../results/medium_model_offline"
...
print("Medium model offline visualization")
print(f"Device: {device}")

dataset = SpatioTemporalDataset(DATA_FILE)

model = MediumCompressor().to(device)
model.load_state_dict(torch.load(f"{MODEL_DIR}/medium_model_offline.pth", map_location=device))
model.eval()
...
plt.suptitle(f'Medium Model (96-96-48) - PSNR: {psnr:.2f} dB - Timestep: {TIMESTEP}',
...
evaluation_results = {
    'model_file': 'medium_model_offline.pth',
...
print("Medium model offline visualization completed")
