"""
Advanced Model Offline Visualization
...
MODEL_DIR = str(PROJECT_ROOT / "results" / "large_model_offline")
OUTPUT_DIR = str(PROJECT_ROOT / "results" / "large_model_offline")
TIMESTEP = 0.0396

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("Advanced model offline visualization")
print(f"Device: {device}")

dataset = FlowDatasetMinMax(DATA_FILE)

model = AdvancedRegressionModel().to(device)
model.load_state_dict(torch.load(f"{MODEL_DIR}/advanced_model_minmax.pth", map_location=device))
model.eval()
...
plt.suptitle(f'Advanced Model (128-128-64) - PSNR: {psnr:.2f} dB - Timestep: {TIMESTEP}',
             fontsize=16, fontweight='bold', y=0.995)
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/advanced_flow_visualization.png", dpi=150, bbox_inches='tight')
print(f"Visualization saved: {OUTPUT_DIR}/advanced_flow_visualization.png")
plt.close()

metrics = {
    'model': 'advanced_model_minmax.pth',
...
print("Advanced model visualization completed")
