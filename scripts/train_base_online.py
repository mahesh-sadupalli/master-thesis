...
OUTPUT_DIR = str(PROJECT_ROOT / "results" / "base_model_online")
...
df.to_csv(f"{OUTPUT_DIR}/base_model_online_metrics.csv", index=False)
print(f"Metrics saved: {OUTPUT_DIR}/base_model_online_metrics.csv")
...
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/base_model_online_progress.png", dpi=300, bbox_inches='tight')
print(f"Training plot saved: {OUTPUT_DIR}/base_model_online_progress.png")
plt.close()

print("Base model online training completed")
